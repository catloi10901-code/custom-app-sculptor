...
const PrayerWallTab = () => {
  ...
  const fetchPrayers = useCallback(async () => {
    let query = supabase.from("prayers").select("*").eq("is_approved", true).order("created_at", { ascending: false }).limit(1000);
  ...
  const toggleAmen = async (prayerId: string) => {
    if (!user) {
      toast.error(t("prayerWall.loginAmen"));
      return;
    }
      ...
      <div className="container">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-8 p-6 sm:p-8 bg-gradient-to-r from-primary/10 via-blue-500/5 to-purple-500/10 border border-primary/20">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <a
                href="#submit-form"
                className="px-5 py-2.5 rounded-lg bg-primary text-primary-foreground font-bold text-sm transition-all duration-300 hover:bg-gold-light shadow-lg shadow-primary/20"
              >
        ...
        {showLetter && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={() => setShowLetter(false)}>
            ...
            <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-card border border-border rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="px-6 py-5 space-y-4 text-foreground text-[0.9rem] leading-relaxed">
                <div className="text-muted-foreground text-sm space-y-1">
                  <p>
                    <strong className="text-foreground">{t("hl.to")}:</strong> {t("hl.toVal")}
                  </p>
                  <p>
                    <strong className="text-foreground">{t("hl.from")}:</strong> {t("hl.fromVal")}
                  </p>
                  <p>
                    <strong className="text-foreground">{t("hl.date")}:</strong> {t("hl.dateVal")}
                  </p>
                </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
            {prayers.map((prayer, i) => (
              <PrayerCard key={prayer.id} prayer={prayer} index={i} hasAmened={amenedPrayers.has(prayer.id)} onToggleAmen={toggleAmen} />
            ))}
          </div>
        )}
