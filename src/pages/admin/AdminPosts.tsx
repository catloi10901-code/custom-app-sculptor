...
const AdminPosts = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (coverFile) {
      ...
      if (upErr) {
        toast.error("Upload ảnh thất bại: " + upErr.message);
        setUploading(false);
        return;
      }
    ...
    if (editingId) {
      ...
      toast.success("Tin tức đã được tạo");
    }
    closeForm();
    fetchData();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-primary text-2xl">Quản Lý Tin Tức</h1>
        <button onClick={showForm ? closeForm : openCreate} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-primary-foreground font-bold text-sm">
          {showForm ? (
            ...
            <>
              <Plus className="w-4 h-4" /> Tạo tin tức
            </>
          )}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-card border border-primary/30 rounded-2xl p-6 mb-6 space-y-4">
          <div className="text-lg font-bold text-foreground mb-2">{editingId ? "✏️ Chỉnh sửa tin tức" : "➕ Tạo tin tức mới"}</div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={uploading} className="px-6 py-2.5 rounded-lg bg-primary text-primary-foreground font-bold text-sm disabled:opacity-60">
              {uploading ? "Đang upload ảnh..." : editingId ? "Cập nhật" : "Lưu tin tức"}
            </button>
          ...
          <tbody>
            {posts.map((p) => (
              <tr key={p.id} className="border-b border-border/50 hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-3 text-center text-muted-foreground text-sm">{new Date(p.created_at).toLocaleDateString("vi-VN")}</td>
                <td className="px-4 py-3 text-center">
