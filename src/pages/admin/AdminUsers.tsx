...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    if (roles) {
      roles.forEach((r) => {
        roleMap[r.user_id] = r.role as "admin" | "moderator" | "user";
      });
    }
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-border/50 hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-3 text-center">
                  <select onChange={(e) => setRole(u.user_id, e.target.value as any)} className="bg-black/20 border border-border rounded px-2 py-1 text-[0.8rem] text-foreground" value={u.role}>
