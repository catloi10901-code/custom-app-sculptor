...
import { BookOpen, HandHeart, Heart, Newspaper, Radio, ScrollText, Target, Users } from "lucide-react";
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {cards.map((c, i) => {
          const Icon = c.icon;
          return (
            <div key={i} className={`relative rounded-2xl border ${c.border} ${c.bg} p-6 flex items-center gap-5 shadow-lg ${c.glow} transition-all duration-300 hover:scale-[1.02] hover:shadow-xl`}>
              ...
              <div className="min-w-0">
                {loading ? <div className="h-8 w-16 bg-white/10 rounded-lg animate-pulse mb-1" /> : <AnimatedCounter value={c.value} className={`font-serif text-3xl font-bold ${c.accent}`} />}
