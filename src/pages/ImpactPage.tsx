...
import { ArrowUpRight, Calendar, Globe, Heart, Loader2, MapPin, RefreshCw, TrendingUp, Users } from "lucide-react";
...
const ImpactPage = () => {
  ...
  const heroBg = useHeroBgImage("hero_bg_impact");
  ...
  useEffect(() => {
    const fetchLive = async () => {
      const { data } = await supabase.from("live_stats").select("prayers_count, members_count, donated_total").eq("id", 1).single();
  ...
  const fetchArticles = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("impact_articles").select("*").eq("is_published", true).order("created_at", { ascending: false }).limit(20);
  ...
  const stories = [
    {
      emoji: "🏫",
      tag: t("impact.stories.1.tag"),
      title: t("impact.stories.1.title"),
      desc: t("impact.stories.1.desc"),
      gradient: "from-blue-500/40 via-indigo-600/30 to-blue-900/50",
      metric: "2,400",
      metricLabel: t("impact.stories.metric.students"),
      accentColor: "#60a5fa",
    },
    {
      emoji: "🏥",
      tag: t("impact.stories.2.tag"),
      title: t("impact.stories.2.title"),
      desc: t("impact.stories.2.desc"),
      gradient: "from-emerald-500/40 via-teal-600/30 to-green-900/50",
      metric: "5,000",
      metricLabel: t("impact.stories.metric.patients"),
      accentColor: "#6ee7b7",
    },
    {
      emoji: "🍚",
      tag: t("impact.stories.3.tag"),
      title: t("impact.stories.3.title"),
      desc: t("impact.stories.3.desc"),
      gradient: "from-amber-500/40 via-orange-600/30 to-yellow-900/50",
      metric: "50,000",
      metricLabel: t("impact.stories.metric.meals"),
      accentColor: "#fbbf24",
    },
    {
      emoji: "💧",
      tag: t("impact.stories.4.tag"),
      title: t("impact.stories.4.title"),
      desc: t("impact.stories.4.desc"),
      gradient: "from-cyan-400/40 via-sky-500/30 to-teal-900/50",
      metric: "8,000",
      metricLabel: t("impact.stories.metric.people"),
      accentColor: "#22d3ee",
    },
  ];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {stats.map((s, i) => {
              ...
              return (
                <div
                  key={i}
                  className="group relative bg-card border border-border rounded-2xl p-7 text-center overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:border-primary/40 hover:shadow-[0_12px_40px_rgba(0,0,0,0.3)]"
                >
                  {/* Glow on hover */}
                  <div
                  ...
                  {/* Icon */}
                  <div
                    className="w-12 h-12 rounded-xl mx-auto mb-4 flex items-center justify-center transition-transform duration-500 group-hover:scale-110"
                  ...
                  <span className="font-serif text-[2.2rem] font-bold text-primary block relative z-10">
                    {s.prefix}
                    <AnimatedCounter value={s.value} duration={1500} className="" />
                    {s.suffix}
                  </span>
        ...
        <div className="container max-w-[900px]">
          ...
          <div className="relative">
            ...
            {milestones.map((m, i) => (
              <div
                key={i}
                className={`relative flex items-start gap-5 mb-8 last:mb-0 ${i % 2 === 0 ? "sm:flex-row" : "sm:flex-row-reverse"}`}
      ...
      <section className="py-16">
        <div className="container">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
            ...
            {isAdmin && (
              <Button onClick={handleFetchNewArticles} disabled={fetching} className="bg-primary text-primary-foreground hover:bg-gold-light gap-2 shadow-[0_4px_15px_rgba(197,160,89,0.25)]">
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`relative h-[140px] bg-gradient-to-br ${config.gradient} flex items-center justify-center overflow-hidden`}>
                      ...
                      {/* Emoji */}
                      <span className="text-5xl relative z-10 transition-transform duration-500 group-hover:scale-110 drop-shadow-[0_4px_8px_rgba(0,0,0,0.3)]">{config.emoji}</span>
                      {/* Topic badge on card */}
                      <div
                        className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[0.65rem] font-bold uppercase tracking-wider backdrop-blur-md border"
                        style={{
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <h3 className="text-foreground text-[1rem] font-semibold mb-2.5 leading-snug line-clamp-2 group-hover:text-primary transition-colors duration-300">{article.title}</h3>
                      <p className={`text-muted-foreground text-[0.84rem] leading-relaxed flex-1 ${isExpanded ? "" : "line-clamp-3"}`}>{article.excerpt || article.content.substring(0, 200)}</p>
        ...
        <div className="container relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
            {stories.map((s, i) => (
              <div
                key={i}
                className="group bg-card border border-border rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:shadow-[0_20px_60px_rgba(0,0,0,0.35)]"
                style={{ animation: `fadeUp 0.6s ease ${0.1 * i}s both` }}
              >
                {/* Rich banner */}
                <div className={`relative h-[200px] bg-gradient-to-br ${s.gradient} flex items-center justify-center overflow-hidden`}>
                  ...
                  {/* Central emoji */}
                  <span className="text-6xl relative z-10 drop-shadow-[0_8px_16px_rgba(0,0,0,0.4)] transition-transform duration-700 group-hover:scale-125 group-hover:rotate-3">{s.emoji}</span>
                  {/* Metric badge */}
                  <div className="absolute bottom-4 right-4 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl px-4 py-2 text-right">
                    <div className="font-serif text-xl font-bold" style={{ color: s.accentColor }}>
                      {s.metric}
                    </div>
                ...
                <div className="p-7">
                  <span
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[0.72rem] font-bold uppercase tracking-wider mb-3"
                    style={{
        ...
        <div className="container max-w-[800px]">
          ...
          <div className="bg-card border border-border rounded-2xl p-8">
                ...
                {allocations.map((a, i) => (
                  <div key={i} className="group">
                    <div className="flex items-center justify-between text-[0.88rem] mb-1.5">
                      ...
                      <span className="font-serif font-bold text-lg" style={{ color: a.color }}>
                        {a.pct}%
                      </span>
                    </div>
            ...
            <div className="text-center pt-4 border-t border-border/50">
              <p className="text-[0.8rem] text-muted-foreground">🔒 {t("impact.fund.note")}</p>
            </div>
          </div>
        </div>
      </section>
