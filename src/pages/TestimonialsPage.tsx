...
import { CheckCircle, ChevronDown as ChevDown, Heart, ImageIcon, Loader2, Share2, Star, Upload, Video, X } from "lucide-react";
...
const getFlag = (code: string) => code.toUpperCase().replace(/./g, (c) => String.fromCodePoint(0x1f1e6 - 65 + c.charCodeAt(0)));
...
// ── Phone code selector ──────────────────────────────────────────────────────
const PhoneInput = ({ code, number, onCode, onNumber }: { code: string; number: string; onCode: (v: string) => void; onNumber: (v: string) => void }) => {
  ...
  return (
    <div className="flex gap-2">
        ...
        {open && (
          <div className="absolute top-full left-0 mt-1 w-64 bg-card border border-border rounded-xl shadow-2xl z-[2100] overflow-hidden">
            <div className="p-2 border-b border-border">
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search country..." className="h-8 text-sm" autoFocus />
            </div>
      ...
      <Input value={number} onChange={(e) => onNumber(e.target.value)} placeholder="912 345 678" type="tel" className="flex-1 min-w-0" />
    </div>
  );
};
...
const TestimonyModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const set = (key: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => setForm((p) => ({ ...p, [key]: e.target.value }));

  const handleFiles = (files: FileList | null) => {
    if (!files) return;
    const allowed = Array.from(files).filter((f) => f.type.startsWith("image/") || f.type.startsWith("video/"));
    if (mediaPreviews.length + allowed.length > 5) {
      toast.error(t("testimonials.maxFiles"));
      return;
    }
  ...
  const uploadMedia = async (): Promise<string[]> => {
    ...
    for (const p of mediaPreviews) {
      ...
      if (error) {
        toast.error(t("testimonials.uploadError"));
        throw error;
      }
  ...
  const handleSubmit = async () => {
    if (!form.full_name.trim()) {
      toast.error(t("testimonials.error.name"));
      return;
    }
    if (!form.title.trim()) {
      toast.error(t("testimonials.error.title"));
      return;
    }
    if (!form.before_prayer.trim()) {
      toast.error(t("testimonials.error.before"));
      return;
    }
    if (!form.after_prayer.trim()) {
      toast.error(t("testimonials.error.after"));
      return;
    }
    if (!form.confirmed) {
      toast.error(t("testimonials.error.confirm"));
      return;
    }
          ...
          {submitted ? (
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-muted-foreground mb-1.5">
                    {t("testimonials.field.testimonyTitle")} <span className="text-destructive">*</span>
                  </label>
                  <Input value={form.title} onChange={set("title")} placeholder={t("testimonials.placeholder.title")} />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-muted-foreground mb-1.5">
                    {t("testimonials.field.before")} <span className="text-destructive">*</span>
                  </label>
                      ...
                      <div key={i} className="relative rounded-lg overflow-hidden border border-border aspect-square bg-black/20">
                        {preview.type === "image" ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center gap-1 p-1">
                            ...
                            <span className="text-[10px] text-muted-foreground/60 truncate w-full text-center">{preview.file.name}</span>
                          </div>
                        )}
                        <button
                          onClick={() => removeMedia(i)}
                          className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 flex items-center justify-center border-none cursor-pointer hover:bg-destructive/80 transition-colors"
                        >
...
const DetailModal = ({ item, onClose }: { item: TestimonySubmission; onClose: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg 2xl:max-w-[800px] max-h-[92vh] flex flex-col bg-card border border-border rounded-2xl shadow-2xl">
        {/* Header */}
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {item.media_urls.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block aspect-square rounded-lg overflow-hidden border border-border hover:opacity-85 transition-opacity">
                    {isVideo(url) ? <video src={url} className="w-full h-full object-cover" muted playsInline /> : <img src={url} alt="" className="w-full h-full object-cover" />}
                  </a>
                ))}
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {submissions.map((item) => (
              <div
                key={item.id}
                className="relative group bg-card border border-border rounded-2xl cursor-pointer flex flex-col justify-between p-4 hover:border-primary/50 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(0,0,0,0.3)] transition-all duration-200 overflow-hidden"
              ...
              >
                ...
                <div className="flex-1 h-0 flex flex-col justify-center gap-1.5 py-2 overflow-hidden">
                  <h3 className="font-serif text-primary text-sm font-semibold leading-snug line-clamp-1">"{item.title}"</h3>
                  {item.after_prayer && <p className="text-xs text-foreground/75 leading-relaxed line-clamp-2">{item.after_prayer}</p>}
                  {item.message && <p className="text-xs text-muted-foreground italic leading-relaxed line-clamp-2">"{item.message}"</p>}
                </div>
